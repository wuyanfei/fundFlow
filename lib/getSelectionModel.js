...
var SelectModel = function() {
		this.setSelectionModel = function(stocks) {
			...
			function(callback) {
				...
				for(var i in stocks) {
					if(i == undefined || i.length != 10) continue;
				...
				http.postByUrl(url, sdata1, function(data) {
					if(data) {
						for(var i in data) {
				...
				http.postByUrl(url, sdata2, function(data) {
					if(data) {
						for(var i in data) {
							value[data[i][0]] = data[i][4];
						}
				...
				var _temp = ['list', [
					['SELECT-MODEL.wenjianzhuizhang', wenjianzhuizhang],
					['SELECT-MODEL.jijinzhuizhang', jijinzhuizhang],
					['SELECT-MODEL.niushihuitiao', niushihuitiao],
					['SELECT-MODEL.niushichaodie', niushichaodie],
					['SELECT-MODEL.niushibuzhang', niushibuzhang],
					['SELECT-MODEL.gaopaodixi', gaopaodixi],
					['SELECT-MODEL.qiangshihuitiao', qiangshihuitiao],
					['SELECT-MODEL.zhendangchaodie', zhendangchaodie],
					['SELECT-MODEL.zhendangbuzhang', zhendangbuzhang],
					['SELECT-MODEL.xiongshichaodie', xiongshichaodie],
					['SELECT-MODEL.xiongshibuzhang', xiongshibuzhang]
				]];
