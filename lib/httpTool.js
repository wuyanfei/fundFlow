  ...
  exports.post = function(data, cback, type) { //type用来表示是推送还是抓取，type为undefined时是推送否则是抓取
    var option = {
      method:'POST',
  ...
  exports.postByUrl = function(address,data, cback) {
    var option = {
      url: address,
      json: data,
      timeout: 20000,
    ...
    request.post(option, function(e, r, body) {
      if(e) {
        log.error('method=post,' + e + ',url=' + address);
        if(cback) {
          cback(null);
        }
      } else {
